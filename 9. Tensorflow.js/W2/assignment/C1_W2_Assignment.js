...
async function train(model, data) {
    ...
    const [trainXs, trainYs] = tf.tidy(() => {
        const dataTraining = data.nextTrainBatch(TRAIN_DATA_SIZE);
        return [
            dataTraining.xs.reshape([TRAIN_DATA_SIZE, 28, 28, 1]),
            dataTraining.labels
        ]
    });
    ...
    const [testXs, testYs] = tf.tidy(() => {
        const dataTesting = data.nextTrainBatch(TEST_DATA_SIZE);
        return [
            dataTesting.xs.reshape([TEST_DATA_SIZE, 28, 28, 1]),
            dataTesting.labels
        ]
    });
